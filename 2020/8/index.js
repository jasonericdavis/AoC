...
const puzzle1 = (instructions) => {
    ...
    while(executedInstructions == [] || !executedInstructions.includes(instructionPointer)){
        const instruction = instructions[instructionPointer]
        const [operation, iterator] = instruction.split(' ')
        
        console.log(`Accumlator = ${accumulator} | operation = ${operation} | iterator = ${iterator}`)
...
const puzzle2 = (instructions) => {
    ...
    while(executedInstructions == [] || (!executedInstructions.includes(instructionPointer) && !executedInstructions.includes(instructions.length - 1))){
        const instruction = instructions[instructionPointer]
        const [operation, iterator] = instruction.split(' ')
