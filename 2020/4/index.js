...
const validator2 = (account) => {
  if (
    !account["byr"] ||
    parseInt(account["byr"]) < 1920 ||
    parseInt(account["byr"]) > 2002
  )
    return false;
  if (
    !account["iyr"] ||
    parseInt(account["iyr"]) < 2010 ||
    parseInt(account["iyr"]) > 2020
  )
    return false;
  if (
    !account["eyr"] ||
    Number(account["eyr"]) < 2020 ||
    Number(account["eyr"]) > 2030
  )
    return false;
  ...
  if (!account["hgt"]) {
    ...
    const height = Number(
      account["hgt"].substring(0, account["hgt"].length - 2)
    );
    if (
  ...
  if (
    !account["ecl"] ||
    !["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].includes(account["ecl"])
  )
    return false;
  if (!account["pid"] || (account["pid"].match(/[0-9]/g) || []).length != 9)
    return false;
  return true;
};
...
try {
  ...
  const accountsString = fs
    .readFileSync("puzzle_input2.txt", "utf8")
    .split("\n\n");
