...
try {  
    const boardingTickes = fs.readFileSync('puzzle_input.txt', 'utf8').split('\n')
    ...
    console.log(seats.sort((a,b) => a.id-b.id)
    .filter((item, index) => {
        if(index === 0) return false
        if(index + 1 > seats.length - 1 ) return false
        return !(seats[index - 1].id === item.id - 1) || !(seats[index + 1].id === item.id + 1) 
    })
    )
